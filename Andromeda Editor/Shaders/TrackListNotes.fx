...
struct NOTE
{
	// 00, channel, key, velocity
	// 0x00CCKKVV
	dword note_data : NOTE_DATA;
	dword tick_start : TICK_START;
	dword tick_end : TICK_END;
};
...
void GS_Note(point NOTE input[1], inout TriangleStream<PS_IN> OutputStream)
{
	...
	float4 cl = float4(1.0, 0.0, 0.0, 1.0);

	v.col = cl;
	v.pos = float4(n.tick_start, TrackTop, 0, 1);
	v.pos.xy = v.pos.xy * 2 - 1;
	v.uv = float2(0.0, (TrackTop - TrackBottom) * (float)ScreenHeight);
	OutputStream.Append(v);
	v.pos = float4(n.tick_start, TrackBottom, 0, 1);
	v.pos.xy = v.pos.xy * 2 - 1;
	v.uv = float2(0.0, 0.0);
	OutputStream.Append(v);
	v.pos = float4(n.tick_end, TrackBottom, 0, 1);
	v.pos.xy = v.pos.xy * 2 - 1;
	v.uv = float2((n.tick_end - n.tick_start) * (float)ScreenWidth, 0.0);
	OutputStream.Append(v);
	OutputStream.RestartStrip();
	...
	v.uv = float2((n.tick_end - n.tick_start) * (float)ScreenWidth, 0.0);
	OutputStream.Append(v);
	v.pos = float4(n.tick_end, TrackTop, 0, 1);
	v.pos.xy = v.pos.xy * 2 - 1;
	v.uv = float2((n.tick_end - n.tick_start) * (float)ScreenWidth, (TrackTop - TrackBottom) * (float)ScreenHeight);
	OutputStream.Append(v);
	v.pos = float4(n.tick_end, TrackTop, 0, 1);
	v.pos.xy = v.pos.xy * 2 - 1;
	v.uv = float2(0.0, (TrackTop - TrackBottom) * (float)ScreenHeight);
